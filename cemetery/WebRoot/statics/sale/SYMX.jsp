...
<html>
  ...
  <body>
     <div class="page_title">销售统计&gt;使用明细</div>
    <div class="button_bar">
	
	<button class="common_button" onclick="to('');">打印</button>  
	
	</div>
	<form  method="post" action="${pageContext.request.contextPath }/showSymx.html"> 
        ...
        <table class="data_list_table">
						...
						<c:forEach var="symx" items="${symxList}" varStatus="status">
							<tr class="" id="id">
								<td>${status.index + 1}</td>
								<td>${symx.mqmc}</td>
								<td>${symx.syqkTotalsh}</td>
								<td>${symx.syqkTotalg}</td>
								<td>${symx.mxlb}</td>
								<td>${symx.ssje}</td>
